...
class HelloViewSets(viewsets.ViewSet):
    ...
    def list(self, request):
        """api for providing list of object"""
        a_view_set = [
            'Uses action (create, list, Retrieve, update, partial_update, delete)',
            'Automatically maps url s using routes',
            'provides more functionality with less code',
        ]
        return Response({'message':'Hello!','a_view_set':a_view_set})

    def create(self, request):
        """ Create a anew hello message"""
        serializer = self.serializer_class(data=request.data)
        if serializer.is_valid():
            name = serializer.validated_data.get('name')
            message = f'Hello {name}'
